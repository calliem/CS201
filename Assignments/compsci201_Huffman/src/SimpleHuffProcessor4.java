...
public class SimpleHuffProcessor4 implements IHuffProcessor {
    ...
    public int preprocessCompress(InputStream in) throws IOException {
        ...
        myWeights = new int[IHuffConstants.ALPH_SIZE+1];
    	
        int current = bis.readBits(IHuffConstants.BITS_PER_WORD);
        while(current != -1){
        	myWeights[current]++;
			inputSize += BITS_PER_WORD;
        ...
        outputSize = BITS_PER_INT*ALPH_SIZE;	//header
		outputSize += BITS_PER_INT;	// magic number
		
       // System.out.println("Map");
        System.out.println("myMap.size() " + myMap.size() + "myWeights.length " + myWeights.length);
        ...
        //	System.out.println("i= " + i);
         ...
         //   System.out.println(myMap.get(i) + " " + (char) k );
			outputSize += (myMap.size())*myWeights[i];
